...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    ...
    fetch(
      `${apiUrl}/notifications?citizen_name=${encodeURIComponent(
        user.name
      )}&audience=citizen`,
      { cache: "no-store" }
    )
    ...
    <div className="mx-auto max-w-2xl space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">
            {tr("Notifications", "सूचनाएँ")}
          </h1>

          <p className="mt-1 text-sm text-slate-500">
            {unreadCount > 0
              ? hindi
                ? `आपके पास ${unreadCount} अपठित ${
                    unreadCount > 1 ? "सूचनाएँ" : "सूचना"
                  } हैं`
              ...
              : tr(
                  ...
                  "आपकी सभी सूचनाएँ देख ली गई हैं!"
                )}
          </p>
        </div>
        ...
        {unreadCount > 0 && (
          ...
          >
            <CheckCheck size={15} />
            {tr("Mark all read", "सभी को पढ़ा हुआ चिह्नित करें")}
          </button>
        )}
      </div>
      ...
      {isLoading && (
        <div className="rounded-2xl border border-slate-100 bg-white p-6 text-sm text-slate-500 shadow-sm">
          {tr("Loading notifications...", "सूचनाएँ लोड हो रही हैं...")}
        </div>
      )}
      ...
      {loadError && (
        <div className="rounded-2xl border border-red-100 bg-red-50 p-6 text-sm text-red-700">
          {tr(
            ...
            "सूचनाएँ लोड नहीं हो सकीं। कृपया रिफ्रेश करके पुनः प्रयास करें।"
          )}
        </div>
      )}
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-100 bg-white py-20 text-center shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-400">
            {tr(
              ...
              "जब आपकी समस्याओं में अपडेट होगा, तो आपको सूचित किया जाएगा।"
            )}
          </p>
        </div>
      )}
  ...
  async function openNotification(notification: Notification) {
    ...
    if (notification.category === "project" && notification.problemId) {
      router.push(
        `/projects/${encodeURIComponent(notification.problemId)}`
      );
      return;
    }
    ...
    router.push(
      notification.problemId
        ? `/my-problems?problemId=${encodeURIComponent(
            notification.problemId
          )}`
        : "/my-problems"
    );
