...
const ALLOWED_ROLES_GET: UserRole[] = ["STUDENT", "MENTOR", "FACULTY", "INDUSTRY_EMPLOYEE", "INDUSTRY_EXPERT", "GOVERNMENT_OFFICER", "ADMIN"]
const ALLOWED_ROLES_POST: UserRole[] = ["STUDENT", "MENTOR", "FACULTY", "INDUSTRY_EMPLOYEE", "INDUSTRY_EXPERT", "GOVERNMENT_OFFICER", "ADMIN"]
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Ensure user can only upload evidence as themselves
    if (userId !== user.id) {
      return NextResponse.json({ error: 'Cannot upload evidence for another user' }, { status: 403 });
    }
