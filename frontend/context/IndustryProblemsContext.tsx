...
const IndustryProblemsContext = createContext<IndustryProblemsContextType | undefined>(undefined);

export function IndustryProblemsProvider({ children }: { children: ReactNode }) {
  ...
  const fetchProblems = async () => {
    ...
    try {
      ...
      if (Array.isArray(records)) {
        setAssignedProblems(records.map(toFrontendProblem));
      }
    } catch (error) {
      console.error("Failed to fetch industry problems:", error);
      setAssignedProblems([]);
    } finally {
      setIsLoading(false);
    }
...
export function useIndustryProblems() {
  const context = useContext(IndustryProblemsContext);
  if (!context) throw new Error("useIndustryProblems must be used inside IndustryProblemsProvider");
  return context;
}