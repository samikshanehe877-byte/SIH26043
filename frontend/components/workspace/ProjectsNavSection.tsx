...
export default function ProjectsNavSection({
  ...
  return (
    <div className="mt-3 border-t border-slate-100 pt-3">
      ...
      >
        <FolderKanban size={17} className={isListActive ? "text-white" : "text-slate-400"} />
        <span className="flex-1">{label}</span>
        {projects.length > 0 && (
          <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${isListActive ? "bg-white/20 text-white" : colors.soft}`}>
            {projects.length}
          </span>
        )}
      </Link>
      ...
      <div className="mt-1 space-y-0.5 pl-3">
        {isLoading && <div className="mx-3 my-1 h-8 animate-pulse rounded-lg bg-slate-100" />}
        {!isLoading && projects.length === 0 && (
          <p className="px-3 py-1.5 text-xs text-slate-400">{emptyText}</p>
        )}
          ...
          return (
            <Link
              ...
              className={`block rounded-lg px-3 py-1.5 text-xs transition ${
                isActive ? `${colors.soft} font-semibold` : "text-slate-500 hover:bg-slate-50 hover:text-slate-800"
              }`}
            >
              <span className="block truncate">{project.title}</span>
              <span className="mt-1 flex items-center gap-2">
                <span className="h-1 flex-1 overflow-hidden rounded-full bg-slate-100">
                  <span className={`block h-full rounded-full ${colors.bar}`} style={{ width: `${project.progress}%` }} />
                </span>
