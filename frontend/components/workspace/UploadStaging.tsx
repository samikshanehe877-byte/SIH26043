...
export function useStagedFiles(maxFiles: number) {
  ...
  const add = (picked: FileList | null) => {
    ...
    for (const file of Array.from(picked)) {
      if (next.some((f) => f.name === file.name && f.size === file.size && f.lastModified === file.lastModified)) {
        refused.push({ name: file.name, reason: "Already selected." });
        continue;
      }
      const problem = attachmentProblem(file);
      if (problem) {
        refused.push({ name: file.name, reason: problem });
        continue;
      }
      if (next.length >= maxFiles) {
        refused.push({ name: file.name, reason: `Only ${maxFiles} file${maxFiles === 1 ? "" : "s"} can be added here.` });
        continue;
      }
      next.push(file);
    }
  ...
  return {
    ...
    rejected,
    add,
    remove: (index: number) => setFiles((prev) => prev.filter((_, i) => i !== index)),
    clear: () => {
      setFiles([]);
      setRejected([]);
    },
...
export function FilePickerButton({
  label, disabled, onPick, className,
}: {
  label: string;
  disabled?: boolean;
...
export function StagedFilesGrid({
  files, phase, failedFileName, onRemove,
}: {
  files: File[];
  phase: UploadPhase;
    ...
    <>
      <ul className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {items.map((item, index) => {
          const isCulprit = phase === "failed" && failedFileName === item.name;
          const status = uploading
            ? { label: "Uploading...", className: "bg-blue-50 text-blue-700", icon: <Loader2 size={11} className="animate-spin" /> }
            : isCulprit
              ? { label: "Rejected", className: "bg-red-100 text-red-700", icon: <AlertTriangle size={11} /> }
              : phase === "failed"
                ? { label: "Not uploaded", className: "bg-amber-50 text-amber-700", icon: <AlertTriangle size={11} /> }
                : { label: "Ready to upload", className: "bg-slate-100 text-slate-600", icon: null };
          return (
            <li
              key={`${item.name}-${item.size}-${index}`}
              className={`relative overflow-hidden rounded-xl border bg-white ${isCulprit ? "border-red-300 ring-2 ring-red-100" : "border-slate-200"}`}
            >
              <button
                type="button"
              ...
              >
                <div className="relative flex aspect-video w-full items-center justify-center overflow-hidden bg-slate-100">
                  {item.src ? <PreviewThumbnail item={item} /> : <Loader2 size={16} className="animate-spin text-slate-400" />}
                  {uploading && <span className="absolute inset-0 bg-white/40" />}
                </div>
                <div className="px-2 py-1.5">
                  <p className="truncate text-xs font-medium text-slate-700">{item.name}</p>
                  <div className="mt-1 flex items-center justify-between gap-1">
                    <span className={`inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-[10px] font-semibold ${status.className}`}>
                      {status.icon}
                      {status.label}
                    </span>
...
export function RejectedFilesNotice({ rejected, onDismiss }: { rejected: RejectedFile[]; onDismiss: () => void }) {
  ...
  return (
    <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
      <div className="flex items-start gap-2">
        ...
        <button onClick={onDismiss} aria-label="Dismiss" className="flex-shrink-0 text-amber-500 hover:text-amber-700"><X size={13} /></button>
      </div>
    </div>
  );
}

export function UploadProgress({ fileCount, fraction }: { fileCount: number; fraction: number }) {
  ...
  return (
    <div className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-2">
      ...
      <p className="mt-1 text-[11px] text-blue-700">Keep this page open until it finishes.</p>
    </div>
  );
}
...
export function UploadFailedNotice({
  ...
  return (
    <div role="alert" className="rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-xs text-red-800">
      <div className="flex items-start gap-2">
...
export function UploadSucceededNotice({
  message, fileNames, onView, onDismiss,
}: {
  message: string;
  fileNames: string[];
  onView?: () => void;
  ...
  return (
    <div role="status" className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2.5 text-xs text-emerald-900">
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 flex-1">
            ...
            <ul className="mt-1 space-y-0.5">
              {fileNames.map((name, index) => (
                <li key={`${name}-${index}`} className="flex items-center gap-1">
                  <CheckCircle2 size={11} className="flex-shrink-0 text-emerald-600" />
          ...
          {onView && (
            <button onClick={onView} className="mt-1.5 font-semibold text-emerald-700 underline hover:text-emerald-900">
              Show it in the updates list
            </button>
          )}
        </div>
        <button onClick={onDismiss} aria-label="Dismiss" className="flex-shrink-0 text-emerald-500 hover:text-emerald-700"><X size={13} /></button>
      </div>
    </div>
  );
}
