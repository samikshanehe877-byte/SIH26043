import { ClipboardList, CheckCircle2, RefreshCw, AlertTriangle, Info } from "lucide-react";
import { UniversityNotification } from "@/types/universityChallenge";

const config = {
  assignment: { icon: ClipboardList, bg: "bg-indigo-50",  iconColor: "text-indigo-500",  border: "border-indigo-100"  },
  success:    { icon: CheckCircle2,  bg: "bg-green-50",   iconColor: "text-green-500",   border: "border-green-100"   },
  update:     { icon: RefreshCw,     bg: "bg-blue-50",    iconColor: "text-blue-500",    border: "border-blue-100"    },
  warning:    { icon: AlertTriangle, bg: "bg-amber-50",   iconColor: "text-amber-500",   border: "border-amber-100"   },
  info:       { icon: Info,          bg: "bg-slate-50",   iconColor: "text-slate-500",   border: "border-slate-100"   },
};

export default function UniversityNotificationItem({
  notification,
}: {
      ...
      <div className="flex-1 min-w-0">
        ...
        {notification.relatedChallengeTitle && (
          <p className="mt-1.5 text-xs font-medium text-indigo-600">
            Re: {notification.relatedChallengeTitle}
          </p>
        )}
      </div>
    </div>
